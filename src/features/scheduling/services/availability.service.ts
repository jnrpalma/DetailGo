...
/**
 * Slots disponíveis para o dia (respeitando capacity).
 * Preferência: status + dayKey
 * Fallback: range por startAtMs para docs antigos sem dayKey.
 */
export async function getAvailableSlotsForDay(day: Date, durationMin: number): Promise<Slot[]> {
  ...
  // não deixa horários no passado (se for hoje)
  const now = Date.now();
...
/**
 * Cria agendamento garantindo capacity (transaction).
 * Salva dayKey + customerName no global e espelha no usuário.
 */
